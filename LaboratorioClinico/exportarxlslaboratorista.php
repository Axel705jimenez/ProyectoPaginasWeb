...
$hojaActiva->setCellValue('E1', 'numTelefono');
$hojaActiva->setCellValue('F1', 'correoElectronico');
$hojaActiva->setCellValue('G1', 'especialidad');
$hojaActiva->setCellValue('H1', 'numLicencia');
...
while($rows = $datos->fetch_assoc())
{
    ...
    $hojaActiva->setCellValue('E'.$Fila, $rows['numTelefono']);
    $hojaActiva->getColumnDimension('F')->setWidth(20);
    $hojaActiva->setCellValue('F'.$Fila, $rows['correoElectronico']);
    $hojaActiva->getColumnDimension('G')->setWidth(20);
    $hojaActiva->setCellValue('G'.$Fila, $rows['especialidad']);
    $hojaActiva->getColumnDimension('H')->setWidth(20);
    $hojaActiva->setCellValue('H'.$Fila, $rows['numLicencia']);
    $Fila++;
}

header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="laboratorista.xlsx"');
